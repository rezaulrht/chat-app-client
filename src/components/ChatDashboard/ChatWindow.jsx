...
export default function ChatWindow({ conversation, onMessageSent }) {
  ...
  if (!conversation) {
    return (
      <div className="flex-1 bg-background-dark flex flex-col items-center justify-center gap-4">
  ...
  return (
    <main className="flex-1 flex flex-col bg-background-dark relative h-full">
      <header className="h-17 border-b border-white/5 flex justify-between items-center px-5 bg-background-dark/80 backdrop-blur-sm shrink-0">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div
              className={`rounded-2xl overflow-hidden ${isParticipantOnline ? "ring-2 ring-primary/60 ring-offset-1 ring-offset-background-dark" : ""}`}
            >
              <Image
                src={
                  participant?.avatar ||
            ...
            <div
              className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-background-dark ${isParticipantOnline ? "bg-green-400" : "bg-slate-600"}`}
            ></div>
          </div>
          <div>
              ...
              >
                ...
                >
                  <div className="relative group w-fit">
                    {!msg.isOptimistic && (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="p-1 text-slate-400 hover:text-primary hover:bg-slate-700/60 rounded-md transition-all duration-150"
                        >
                          <Smile size={14} />
                        </button>
                        <button
                    ...
                    <div
                      className={`${isGif ? "p-1" : "p-3.5"} rounded-2xl text-[13px] leading-relaxed relative z-10 ${isMe ? (isGif ? "bg-transparent" : "bg-primary text-background-dark font-medium rounded-br-none shadow-lg shadow-primary/10") : isGif ? "bg-transparent" : "bg-surface-dark text-slate-200 border border-white/5 rounded-bl-none shadow-sm shadow-black/5"} ${msg.isOptimistic ? "opacity-60" : ""}`}
                    >
                      {msg.replyTo && (
                        <div className="mb-2 p-2 bg-black/20 rounded-lg border-l-2 border-primary text-[11px] opacity-80 line-clamp-2">
                          <p className="font-bold mb-0.5 whitespace-nowrap overflow-hidden text-ellipsis">
                  ...
                  {isMe && !msg.isOptimistic && (
                    <div className="flex items-center gap-0.5 px-0.5 mt-0.5">
                      ...
                      {msg.status === "read" && (
                        <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/15 text-primary text-[8px] font-semibold">
                          <svg
                            width="10"
                            height="8"
                            viewBox="0 0 16 12"
      ...
      <form
        onSubmit={handleSend}
        className="px-4 pb-6 pt-2 relative bg-background-dark border-t border-white/5 z-50 flex-none"
      >
        {replyTo && (
          <div className="absolute bottom-full left-0 right-0 p-3 bg-surface-dark border-t border-primary/30 flex items-center justify-between animate-in slide-in-from-bottom-2 fade-in">
        ...
        <div className="bg-[#1e252e] rounded-lg p-2 flex flex-col gap-2 border border-white/5 focus-within:ring-1 focus-within:ring-primary/50 transition-all shadow-lg">
          <div className="flex items-center gap-2 border-b border-white/5 pb-2 px-1">
            <button
              type="button"
              className="text-text-secondary-dark hover:text-white transition-colors p-1 rounded hover:bg-white/5"
            >
          ...
          <div className="flex gap-4 items-end">
            <button
              type="button"
              className="text-text-secondary-dark hover:text-white transition-colors mb-1 ml-1 p-1 rounded-full hover:bg-white/10 flex-none"
            >
              <Plus size={24} />
            </button>
            <div className="flex-1 relative">
              <input
                className="w-full bg-transparent border-none text-white placeholder-text-secondary-dark/50 focus:ring-0 resize-none py-2 px-0 leading-relaxed outline-none"
              ...
              {suggestions.length > 0 && (
                <div className="absolute bottom-12 left-0 bg-[#15191C]/95 backdrop-blur-md border border-slate-800 rounded-xl p-1 shadow-2xl z-50 min-w-37.5">
                  {suggestions.map(([code, emoji], i) => (
                    <div
                      key={code}
                      onClick={() => insertEmoji(emoji)}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${i === suggestionIndex ? "bg-primary/20 text-primary" : "hover:bg-slate-800 text-slate-400"}`}
                    >
            ...
            <div className="flex items-center gap-2 mb-1 mr-1 flex-none">
              <button
                type="button"
                onClick={() => {
                ...
                className={`text-text-secondary-dark hover:text-white transition-colors p-1 rounded flex items-center justify-center ${showGifPicker ? "text-primary bg-primary/10" : "hover:bg-white/10"}`}
              >
                <div className="text-[10px] font-black border border-current rounded px-1 min-w-7.5 text-center">
                  GIF
                </div>
              </button>
              <button
                ...
                className={`text-text-secondary-dark hover:text-white transition-colors p-1 rounded hover:bg-white/10 ${showEmojiPicker ? "text-primary" : ""}`}
              >
                <Smile size={24} />
              </button>
              <button
                type="submit"
                className="bg-primary hover:bg-primary/80 text-background-dark rounded p-1.5 transition-colors flex items-center justify-center shadow-lg shadow-primary/20"
              >
                <Send size={18} />
              </button>
            </div>
