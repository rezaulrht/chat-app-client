  ...
  return (
    <aside className="w-full sm:w-76 shrink-0 flex flex-col h-full bg-slate-surface/90 backdrop-blur-2xl md:border-l border-white/[0.08] overflow-hidden">
      ...
      <div className="h-[68px] flex items-center justify-between px-5 border-b border-white/[0.06] shrink-0 relative">
        ...
        <button
          onClick={onClose}
          className="w-7 h-7 rounded-xl hover:bg-white/[0.06] flex items-center justify-center text-ivory/20 hover:text-ivory/50 transition-all"
        >
          <X size={14} />
        </button>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
        ...
        <div className="flex flex-col items-center gap-3 py-8 px-5 border-b border-white/[0.06] relative">
          ...
          <Link
            ...
            className="relative z-10 group/av"
          >
            <div
              className="w-20 h-20 rounded-2xl overflow-hidden ring-4 ring-offset-4 ring-offset-obsidian shadow-lg transition-transform group-hover/av:scale-105"
              style={{ borderColor: themeColor, "--tw-ring-color": themeColor }}
            >
              <Image
                src={
                  participant?.avatar ||
          ...
          <div className="z-10 text-center">
            <p className="text-ivory font-display font-bold text-base leading-tight">
              {displayName}
            </p>
            {prefs.nickname && (
            ...
            {[
              {
                icon: Phone,
                label: "Voice",
                action: () => startCall(participant?._id, "voice"),
              },
              {
                icon: Video,
                label: "Video",
                action: () => startCall(participant?._id, "video"),
              },
              ...
              >
                ...
                <span className="text-[9px] font-mono text-ivory/25">
                  {label}
                </span>
              </button>
            ))}
              ...
              {editingNick ? (
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    className="flex-1 bg-transparent text-ivory/80 text-sm outline-none pb-0.5 placeholder:text-ivory/20"
                    style={{ borderBottom: `1px solid ${themeColor}80` }}
                  />
                  <button onClick={saveNick} style={{ color: themeColor }}>
                    <Check size={14} />
                  </button>
                </div>
              ) : (
                <button
                  ...
                  className="text-ivory/60 text-sm font-medium text-left w-full flex items-center justify-between group"
                >
                  <span className="truncate">
                    {prefs.nickname || (
                      <span className="text-ivory/25 font-mono text-[11px]">
                        Set nickname…
                      </span>
                    )}
                  </span>
                  <Pencil
                    size={11}
                    className="text-ivory/20 group-hover:text-accent/50 shrink-0 ml-2 transition-colors"
                  />
                </button>
              )}
            </div>
        ...
        <div className="px-5 py-4">
          <Link
            href={`/profile/${participant?._id}`}
            className="flex items-center justify-between px-3 py-3 rounded-xl hover:bg-white/[0.04] transition-all group"
          >
            ...
            <ChevronRight
              size={13}
              className="text-ivory/20 group-hover:text-ivory/50 shrink-0 transition-colors"
            />
          </Link>
        </div>
      </div>
