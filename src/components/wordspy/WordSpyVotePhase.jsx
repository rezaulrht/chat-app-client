  ...
  return (
    <div className="flex flex-col h-full p-6 gap-4">
      ...
      <div className="flex-1 overflow-y-auto space-y-3">
        ...
        <p className="text-ivory/40 text-xs uppercase tracking-widest mt-4">
          Vote
        </p>
        {connected
          .filter((p) => String(p.userId) !== myId)
          .map((p) => (
            <button
              key={String(p.userId)}
            ...
            >
              ...
              {voted === String(p.userId) && (
                <span className="ml-auto text-accent text-xs">Your vote</span>
              )}
            </button>
          ))}
      </div>
