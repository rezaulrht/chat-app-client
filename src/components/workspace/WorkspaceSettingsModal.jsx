      ...
      <div>
        ...
        >
          {!bannerUrl && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-ivory/20 text-[10px] font-mono">Paste a banner URL below</p>
              </div>
            </div>
          )}
          {/* Avatar overlay */}
          <div className="absolute left-4 -bottom-8 w-16 h-16 rounded-2xl overflow-hidden ring-4 ring-deep bg-accent/10">
            {avatarUrl ? (
              <Image src={avatarUrl} width={64} height={64} alt="avatar" className="object-cover w-full h-full" unoptimized />
            ) : (
              <div className="w-full h-full flex items-center justify-center">
                <span className="text-lg font-bold text-accent/60">{workspace?.name?.[0]?.toUpperCase()}</span>
              </div>
            )}
          </div>
        </div>
...
function RolesTab({ workspace, onCreateRole, onUpdateRole, onDeleteRole }) {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await onCreateRole({ name: newName.trim(), color: newColor });
      setNewName("");
  ...
  const handleUpdate = async (roleId) => {
    ...
    setSaving(true);
    try {
      await onUpdateRole(roleId, { name: editName.trim(), color: editColor });
      setEditingId(null);
  ...
  return (
    <div className="space-y-5 max-w-lg">
      ...
      {roles.length > 0 && (
        <div className="space-y-2">
          <p className="text-[10px] font-mono font-bold text-ivory/25 uppercase tracking-widest">Custom roles</p>
          {roles.map((role) => (
            <div key={role._id} className="px-3 py-2.5 rounded-xl bg-white/3 border border-white/6">
              {editingId === role._id ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => { setEditingId(role._id); setEditName(role.name); setEditColor(role.color); }}
          ...
          return (
            <div key={m.user?._id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/3 transition-colors group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] font-mono text-ivory/25 truncate">{m.user?.email}</p>
              </div>
