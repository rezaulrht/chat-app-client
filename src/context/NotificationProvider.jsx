...
function formatToastMessage(notif) {
  ...
  else if (actors.length > 2) a = `${actors[0].name}, ${actors[1].name} and ${notif.actorCount - 2} others`;
...
export function NotificationProvider({ children }) {
  ...
  const fetchNotifications = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const { notifications: incoming, unreadCount: count, hasMore: more } = res.data;
      if (page === 1) {
      ...
      console.error("fetchNotifications error:", err?.response?.data?.message || err.message);
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const fetchPrefs = useCallback(async () => {
    ...
    try {
      ...
      console.error("fetchPrefs error:", err?.response?.data?.message || err.message);
    }
  }, [user]);
  ...
  const markRead = useCallback(async (id) => {
    ...
    try {
      ...
      console.error("markRead error:", err?.response?.data?.message || err.message);
  ...
  const markAllRead = useCallback(async () => {
    ...
    try {
      ...
      console.error("markAllRead error:", err?.response?.data?.message || err.message);
      fetchNotifications(1);
    }
  }, [fetchNotifications]);
  ...
  const deleteNotif = useCallback(async (id) => {
    ...
    try {
      ...
      console.error("deleteNotif error:", err?.response?.data?.message || err.message);
      fetchNotifications(1);
    }
  }, [fetchNotifications]);
  ...
  const updatePrefs = useCallback(async (newPrefs) => {
    ...
    try {
      ...
      console.error("updatePrefs error:", err?.response?.data?.message || err.message);
      fetchPrefs();
    }
  }, [fetchPrefs]);
